    ...
    return (
        <>
            <div className='flex flex-col justify-center items-center rounded-lg bg-white shadow-xl shadow-black/5 ring-1 ring-slate-700/10'>
                <div className='flex flex-row space-x-3 py-3 w-full px-3'>
                    <UrlTile
                        icon={<AnchorIcon />}
                        text={"From URL"}
                        onUrlUpdate={(e) => {
...
function UrlTile({
    ...
    const [showModal, setShowModal] = useState(false);
    const [urlInput, setUrlInput] = useState(Constants.DEFAULT_AUDIO_URL);
  
    const onChange = (event: React.ChangeEvent<HTMLInputElement>) => {
      event.preventDefault(); // Add this line to prevent form submission (browser refresh)
    ...
    return (
      <>
        ...
        <UrlModal
          show={showModal}
          onSubmit={onUrlUpdate}
          onClose={() => {
            setShowModal(false);
            setUrlInput(Constants.DEFAULT_AUDIO_URL);
          }}
        />
